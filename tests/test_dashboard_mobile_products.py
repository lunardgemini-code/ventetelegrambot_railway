...
class DashboardMobileProductTests(unittest.TestCase):
    ...
    def test_pwa_cache_uses_the_pixel_asset_version(self):
        # The Pixel control center changes both app logic and operations CSS,
        # so both URLs and the app-shell name must advance together.
        self.assertIn("operations.css?v=20260729-pixel-v1", self.html)
        self.assertIn("app.js?v=20260729-pixel-v1", self.html)
        self.assertIn("ventebot-dashboard-shell-20260729-pixel-v1", self.worker)
        self.assertIn("operations.css?v=20260729-pixel-v1", self.worker)
        self.assertIn("app.js?v=20260729-pixel-v1", self.worker)
